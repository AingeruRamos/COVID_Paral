...
int main(int argc, const char * argv[])
{
    ...
    do {
		...
		switch (opcion) {
            ...
            case 'E':
				...
				if (!estaVacia(raiz)) {
					do {
                		printf("\nIntroduce una posición como entero: ");
                		scanf("%d%*c", &indice);
					} while (indice < 0 || indice >= mostrarNumeroNodos(&raiz));
					...
					do {
                		printf("\nIntroduce una posición como entero: ");
                		scanf("%d%*c", &indice);
					} while (indice < 0 || indice >= mostrarNumeroNodos(&raizOrdenada));

                	printf("\nEl valor almacendo en la posición %d es %d\n", indice,devolverInfoPosicionIndice(raizOrdenada, indice));
				}
                break;
            case 'F':
				if (!estaVacia(raiz))
					...
					printf("\n\nEliminando nodo comienzo raiz ordenada: %d", eliminarNodoComienzo(&raizOrdenada));				
				break;
			case 'G':
				if (!estaVacia(raiz))
					printf("\n\nEliminando nodo final: %d", eliminarNodoFinal(&raiz));
				else
					printf("\n\nEliminando nodo final raiz ordenada: %d", eliminarNodoFinal(&raizOrdenada));
				break;
			case 'H':
                	printf("\nIntroduce una posición como entero: ");
                	scanf("%d%*c", &indice);
				if (!estaVacia(raiz))
					printf("\n\nEliminando nodo intermedio: %d", eliminarNodoPosicionIndice(&raiz, indice));
				else
					printf("\n\nEliminando nodo final raiz ordenada: %d", eliminarNodoPosicionIndice(&raizOrdenada, indice));
				break;
                //                aux = raiz;
                //                i=0;
                //                while (aux != NULL && i++ < indice) {
			...
			case 'L':
				if (!estaVacia(raiz))
					printf("\n\nMostrando numero de nodos: %d\n", mostrarNumeroNodos(&raiz));
				else
					printf("\n\nMostrando numero de nodos: %d\n", mostrarNumeroNodos(&raizOrdenada));
				break;
            case 'Q':
				printf("\n\nSaliendo.\n\n");
            case 'K':
